...
interface FormData {
  ...
  // Capacity (4 params) - Max 40 poin (10 poin per param)
  dsr: 0 | 6 | 10 | '';
...
export default function Home() {
  ...
  const metrics = useMemo(() => {
    ...
    const bopo = totalPendapatanOperasional > 0 ? (totalBiayaOperasional / totalPendapatanOperasional) * 100 : 0;

    return { dbr, dsr, ltv, der, bopo, cicilan, plafon };
  }, [formData]);
  ...
  }, [metrics, formData.jenisPekerjaan]);
  ...
  const handleProcess = () => {
    ...
    const masaKerja = formData.masaKerjaTahun || 0;
    if (masaKerja > 5) {
      masaKerjaScore = 10;
    } else if (masaKerja >= 2) {
      masaKerjaScore = 6;
    ...
    // Total max = 190 (Character 60 + Capacity 40 + Capital 30 + Collateral 20 + Condition 40)
    let decision: 'approved' | 'considered' | 'risky' | 'rejected' = 'rejected';
    if (total >= 145) decision = 'approved';      // >= ~76% dari 190
    else if (total >= 120) decision = 'considered'; // >= ~63% dari 190
    else if (total >= 95) decision = 'risky';       // >= ~50% dari 190
            ...
            <div className="col-span-2 space-y-6">
              ...
              <Card className="p-6 bg-white border border-gray-200 rounded">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('masaKerjaTahun', e.target.value === '' ? '' : parseFloat(e.target.value) || '')}
                    className="px-3 py-2 border border-gray-300 rounded bg-white text-gray-900"
                  />
                </div>
              </Card>
              ...
              <Card className="p-6 bg-white border border-gray-200 rounded">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Total Biaya Operasional</label>
                    <input
                      type="text"
                      placeholder="Rp0,00"
                      value={formatRupiah(formData.totalBiayaOperasional)}
                      onChange={(e) => handleMoneyInput('totalBiayaOperasional', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded bg-white text-gray-900"
                    />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-700">Total Pendapatan Operasional</label>
                    <input
                      type="text"
                      placeholder="Rp0,00"
                      value={formatRupiah(formData.totalPendapatanOperasional)}
                      onChange={(e) => handleMoneyInput('totalPendapatanOperasional', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded bg-white text-gray-900"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleInputChange('tenor', e.target.value === '' ? '' : parseFloat(e.target.value) || '')}
                      className="w-full px-3 py-2 border border-gray-300 rounded bg-white text-gray-900"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="0"
                      value={formData.sukuBunga === '' ? '' : formData.sukuBunga}
                      onChange={(e) => handleInputChange('sukuBunga', e.target.value === '' ? '' : parseFloat(e.target.value) || '')}
                      className="w-full px-3 py-2 border border-gray-300 rounded bg-white text-gray-900"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded p-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">Nilai terukur: {metrics.bopo.toFixed(2)}%</p>
                    </div>
                  </div>
                </div>
              </Card>
            ...
            <ul className="space-y-3">
              {(result.decision === 'approved' || result.decision === 'considered') ? (
                <>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                    <span className="text-gray-700">Siapkan dokumen lengkap untuk pengajuan KPR (KTP, slip gaji, rekening koran 3 bulan terakhir).</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                    <span className="text-gray-700">Pertahankan riwayat kredit yang baik selama proses pengajuan.</span>
                  </li>
                  {result.metrics.dbr > 30 && (
                    <li className="flex items-start gap-3">
                      <CheckCircle2 className="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" />
                      <span className="text-gray-700">Pertimbangkan untuk melunasi cicilan lain terlebih dahulu untuk menurunkan DBR.</span>
                    </li>
                  )}
                  {result.metrics.ltv > 80 && (
                    <li className="flex items-start gap-3">
                      <CheckCircle2 className="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" />
                      <span className="text-gray-700">Tambahkan uang muka jika memungkinkan untuk mendapat suku bunga lebih baik.</span>
                    </li>
                  )}
                </>
              ) : (
